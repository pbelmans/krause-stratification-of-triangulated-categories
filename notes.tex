...
\author{Henning Krause\\[1ex] Notes by Pieter Belmans\footnote{Based
    on a lecture series at the Summer School ``Derived categories'',  June 23rd to 27th 2014 in Nantes.}}
...
\begin{theorem}
  ...
  \begin{enumerate}
    \item $H$ is cohomological, i.e.\ it sends exact triangles in~$\mathcal{T}$ to exact sequences and it sends coproducts in~$\mathcal{T}$ to products;
...
\end{remark}
\begin{proposition}
  For a thick subcategory~$\mathcal{S}$ of~$\mathcal{T}$ the following are equivalent:
  \begin{enumerate}
...
\end{definition}
The case~$\alpha=\aleph_0$ yields the usual definition of a compactly generated category.
\begin{theorem}
...
\end{corollary}
\begin{exercise}
  The corollary is due to Neeman; find this important result in his book on triangulated categories (it's well hidden, unfortunately).
\end{exercise}
\begin{theorem}
...
Recall that for a commutative noetherian ring~$A$ we have the associated (big) derived category~$\derived(A)\coloneqq\derived(\Mod/A)$, and to an object~$X$ in~$\derived(A)$ we have associated a support~$\supp X$ and cosupport~$\cosupp X$ which are subsets of~$\Spec A$. This way we parametrise localising and colocalising subcategories. More precisely, the main results from the first lecture are:
\begin{theorem}
...
Fix a~$\mathbb{Z}$\dash graded commutative ring~$R=\bigoplus_{n\in\mathbb{Z}}R^n$, i.e.\ we have~$rs=(-1)^{|r||s|}sr$ for~$r$ and~$s$ homogeneous elements. This graded-commutativity enters the picture because we will often take~$R$ to be a cohomology ring.
\begin{definition}
...
\begin{lemma}
  ...
  \begin{equation}
    \Gamma_V\circ\LL_W\cong\Gamma_{\mathfrak{p}}\cong\LL_W\circ\Gamma_V.
  \end{equation}
\end{lemma}
...
\begin{theorem}
  ...
  \begin{equation}
    \supp_R
    X=\bigcup_{C\in\mathcal{T}^\cc}\min\{\Supp_R\Hom_{\mathcal{T}}^\bullet(C,X)\}
  \end{equation}
  where for an~$R$-module~$M$
  \begin{equation}
    \Supp_R M\coloneqq\left\{ \mathfrak{p}\in\Spec R\mid M_{\mathfrak{p}}\neq 0 \right\}
  \end{equation}
  and a subset~$\mathcal{U}\subseteq\Spec R$
  \begin{equation}
...
\begin{proposition}
  ...
  \begin{enumerate}
    \item The local-to-global principle holds whenever the Krull dimension of~$R$ is finite or when~$\mathcal{T}$ ``has a model.''
    \item Suppose that the local-to-global-principle holds. Then we have bijections
      \begin{equation}
        \begin{tikzcd}
          ...
          \left\{ \substack{\displaystyle\text{collections of localising subcategories of~$\mathcal{T}_{\mathfrak{p}}$} \\ \displaystyle\text{for all $\mathfrak{p}\in\Spec R$}} \right\} & (\mathcal{S}\cap\Gamma_{\mathfrak{p}}(\mathcal{T}))_{\mathfrak{p}\in\Spec R}.
        \end{tikzcd}
      \end{equation}
  \end{enumerate}
\end{proposition}
...
\begin{lemma}
  Let~$X$ be an object of~$\mathcal{T}$. If~$\End_{\mathcal{T}}^\bullet(X)$ is finitely generated over~$R$ then~$\supp_R X=\VV(\mathfrak{a})$, for~$\mathfrak{a}\coloneqq\ker(R\to\End_{\mathcal{T}}^\bullet(X))$.
\end{lemma}
...
\begin{theorem}
  ...
  \begin{equation}
    \begin{tikzcd}
      ...
      \left\{ \text{specialisation-closed subsets of $\supp_R(\mathcal{T})$} \right\} & \supp_R(\mathcal{S}).
    \end{tikzcd}
  \end{equation}
\end{theorem}
\begin{corollary}
...
\begin{example}
  \begin{enumerate}
    \item If~$(\mathcal{T},\otimes,\mathbf{1})$ is a tensor triangulated category then~$\mathcal{T}$ is~$\End_{\mathcal{T}}^\bullet(\mathbf{1})$-linear, and we obtain a classification of tensor ideal localising subcategories. This generalises the classification of localising subcategories to any finite group~$G$, without the assumption on the order of~$G$.
    \item Let~$A$ be a finite-dimensional~$k$\dash algebra. If we wish to stratify~$\derived(A)$ we may run into the following problems: 
      \begin{enumerate}
        \item The category of compacts is \emph{too small}. Take~$A=kG$ for~$G$ a finite group, then~$\derived(A)^\cc\cong \derived^\bounded(\proj/A)$, but we'd rather study~$\stmod(A)$;
        \item The graded center is \emph{too small}.  Take~$A=kQ$ for~$Q$ an acyclic quiver, then~$\centre^\bullet(\derived(A))=k$ if~$Q$ is Dynkin, which is of no use to us.
      \end{enumerate}
...
There is a remarkable consequence, given that taking products in $\Qcoh/X$ in general might not be exact.

\begin{corollary}
  A product of acyclic complexes of injectives is again acyclic.
\end{corollary}
...
\begin{remark}
  ...
  \begin{equation}
    \begin{aligned}
      \image Q_\lambda&=\prescript{\perp}{}{\SSing(\Qcoh/X)}, \\
      \image Q_\rho&=\SSing(\Qcoh/X)^\perp=\text{K-injective complexes}.
    \end{aligned}
  \end{equation}
\end{remark}
...
The following lemma is useful for both the exercise and the subsequent corollary.

\begin{lemma}
  Given an adjoint pair of exact functors between compactly generated triangulated categories, the left adjoint preserves compactness if and only if the right adjoint preserves coproducts.
\end{lemma}
...
\begin{proposition}
  ...
  \begin{equation}
    \begin{tikzcd}[column sep = large]
      \KKK(\mathrm{Proj}/A) \arrow{r}{\Hom_A(-,A)} & \KKK(\Mod/A) \arrow[two heads]{r} & \derived(\Mod/A) \\
...
This problem boils down to describing the indecomposables. The category is hereditary and the indecomposables are the so called ``stalk complexes'', i.e.\ up to shift we have that they are
\begin{equation}
...
\begin{lemma}
  ...
  \begin{equation}
    \Thick(X\sslash\mathfrak{a})=\left\{ Y\in\Thick(X)\mid\End_{\mathcal{T}}^\bullet(Y)_\mathfrak{p}=0\forall\mathfrak{p}\nsupseteq\mathfrak{a} \right\}.
  \end{equation}
\end{lemma}
...
\begin{equation}
  \Gamma_{\mathfrak{p}}(\mathcal{T})\coloneqq\left\{ X\in\mathcal{T}_{\mathfrak{p}}\mid\End_{\mathcal{T}}^\bullet(X)_{\mathfrak{q}}=0\forall\mathfrak{q}\nsupseteq\mathfrak{p} \right\},
\end{equation}
...
\begin{proposition}
  Assume that~$\End_{\mathcal{T}}^\bullet(X)$ is finitely generated over~$R$ for all objects~$X$ of~$\mathcal{T}$. If~$\mathcal{T}$ is not stratified then there exists a pair of objects~$X$ and~$Y$ in~$\mathcal{T}$ such that~$\supp_R X=\supp_R Y$ but~$\Thick(X)\neq\Thick(Y)$.
\end{proposition}

As a final remark: the results of day 3 depend on localisation techniques which are based on the so-called infinite methods, such as homotopy colimits and Brown representability, whereas today's results use categories of cohomological functors to make things work.

\clearpage

\section{References to the literature}

Day 1: \cite{BIK2012a,Ne1992,Ne2011}

Day 2: \cite{BIK2012b,IK2013,Ne2001}

Day 3: \cite{BIK2008,BIK2011}

Day 4: \cite{IK2006,Kr2005,Mu2007,St2011}

Day 5: \cite{BIK2013,Kr2013}

\begin{thebibliography}{99}

\bibitem{BIK2008} D.~J. Benson, S.~B. Iyengar, and H.~Krause, Local
  cohomology and support for triangulated categories, Ann.\ Scient.\
  \'Ec.\ Norm.\ Sup.\ (4) \textbf{41} (2008), 1--47.

\bibitem{BIK2011} D. Benson, S. B. Iyengar\ and\ H. Krause, Stratifying triangulated
categories, J. Topol. {\bf 4} (2011), no.~3, 641--666.

\bibitem{BIK2012a} D. J. Benson, S. B. Iyengar\ and\ H. Krause, Colocalizing
subcategories and cosupport, J. Reine Angew. Math. {\bf 673} (2012),
161--207.

\bibitem{BIK2012b} D. J. Benson, S. Iyengar\ and\ H. Krause, {\it
    Representations of finite groups: local cohomology and support},
  Oberwolfach Seminars, 43, Birkh\"auser/Springer Basel AG, Basel,
  2012.

\bibitem{BIK2013} D. J. Benson, S. B. Iyengar\ and\ H. Krause, A
  local-global principle for small triangulated categories,
  arXiv:1305.1668 (to appear in Math.\ Proc.\ Camb.\ Phil.\ Soc.).

\bibitem{IK2006} S. Iyengar\ and\ H. Krause, Acyclicity versus total acyclicity for
complexes over Noetherian rings, Doc. Math. {\bf 11} (2006), 207--240
(electronic).

\bibitem{IK2013} S. B. Iyengar\ and\ H. Krause, The Bousfield lattice of a triangulated
category and stratification, Math. Z. {\bf 273} (2013), no.~3-4,
1215--1241.

\bibitem{Kr2005} H.~Krause, The stable derived category of a
  noetherian scheme, Compositio Math. \textbf{141} (2005), 1128--1162.

\bibitem{Kr2013} H. Krause, Report on locally finite triangulated
  categories, J. K-Theory {\bf 9} (2012), no.~3, 421--458.

\bibitem{Mu2007} D. Murfet, The Mock Homotopy Category of Projectives
  and Grothendieck Duality, PhD thesis, Australian National
  University, 2007.

\bibitem{Ne1992} A.~Neeman, The chromatic tower for $\mathbf{D}(R)$, Topology
  \textbf{31} (1992), 519--532.

\bibitem{Ne2001} A.~Neeman, \emph{Triangulated categories}, Annals of
  Mathematics Studies 148, Princeton University Press, 2001.

\bibitem{Ne2011}
A. Neeman, Colocalizing subcategories of $\mathbf{D}(R)$, J. Reine
Angew. Math. {\bf 653} (2011), 221--243.

\bibitem{St2011} G. Stevenson, Subcategories of singularity categories
  via tensor actions, arXiv:1105.4698 (to appear in Compositio Math.).
